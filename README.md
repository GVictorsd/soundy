...
- <strong>Low pass filter</strong>: Allows low frequency signals to pass through
- <strong>High pass filter</strong>: Allows high frequency signals to pass through
- <strong>Band pass filter</strong>: Allows frequencies of certain range to pass through

### Low Pass Filter
With that said, let's consider only a low pass filter for now.
A simple low pass filter can be constructed as shown. It is built using a resistor and capacitor pair. Such setup is often refered as a first order low pass filter.
...
```
Make sure python is installed in your machine. The code requires python modules like numpy and scipy. Make sure to install them too. A simple `pip install ` will do it. The above should create a new file named `rec.wav`. Any .wav running software like VLC player should be able to read it and play the sound. Personally, I use the following python script.
